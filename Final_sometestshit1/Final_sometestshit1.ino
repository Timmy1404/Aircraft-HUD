...
tft.println(1); 
tft.println(Coordinates[0][0]); 
tft.println(Coordinates[0][1]); 
tft.println(Coordinates[0][2]); 
tft.println(Coordinates[0][3]); 
tft.println(""); 

tft.println(2); 
tft.println(Coordinates[1][0]); 
tft.println(Coordinates[1][1]); 
tft.println(Coordinates[1][2]); 
tft.println(Coordinates[1][3]); 

//tft.drawLine(Coordinates[0][2], Coordinates[0][3], Coordinates[1][2], Coordinates[1][3], ST77XX_GREEN);

//draw all static display elements
tft.fillScreen(ST77XX_BLACK);
  tft.drawCircle(120, 120, 5, ST77XX_GREEN);
  //tft.drawCircle(Coordinates[1][2], Coordinates[1][3], 5, ST77XX_RED);
  //tft.drawCircle(Coordinates[2][2], Coordinates[2][3], 5, ST77XX_BLUE);
...
double NewCoordinates(double xO, double yO, double &xN, double &yN, double x, double y) {  //& = refrences value
  xO = xO - 120;
  yO = yO - 120;
  xN = sqrt((xO * xO) + (yO * yO)) * sin(((x + ((atan((xO) / (yO))) / M_PI) * 180) / 180) * M_PI);
  yN = sqrt((xO * xO) + (yO * yO)) * cos(((x + ((atan((xO) / (yO))) / M_PI) * 180) / 180) * M_PI);
  xN = xN + (2 * y * sin((x * M_PI) / 180));
  yN = yN + (2 * y * cos((x * M_PI) / 180));
  xN = xN + 120; 
  yN = yN + 120;
}