...
int Coordinates[ROWS][COLUMNS] = { 
  {60, 125, 0, 0},     //line1 start - old and new x and y values
  {180, 125, 0, 0},    //line1 end - old and new x and y values
  {60, 120+40, 0, 0},     //line2 start
  {180, 120+40, 0, 0}    //line2 end
  };
...
int DrawElements(Adafruit_ST7789 tft, int Coordinates[][4], int ROWS) {
  ...
  for (int i = 0; i < ROWS; i++) {
    ...
    if ((i == 0 || (i % 2 == 0)) && (60 < Coordinates[i][3] && Coordinates[i][3] < 180)) { 
      ...
      //tft.setCursor(INDEX1, 0);
      //tft.println(Coordinates[i][2]); 
      //tft.println(Coordinates[i][3]); 
      //tft.println(Coordinates[i+1][2]); 
      //tft.println(Coordinates[i+1][3]); 


      tft.setCursor(INDEX1, INDEX2);
      tft.println(Coordinates[i][2]); 
      INDEX2 = INDEX2 + 10;
      //tft.println(""); 
      tft.setCursor(INDEX1, INDEX2);
      ...
      tft.drawLine(X1, Y1, X2, Y2, ST77XX_MAGENTA);
      tft.drawCircle(X1, Y1, 5, ST77XX_MAGENTA);

      //tft.drawLine(Coordinates[i][2], 120-(Coordinates[i][3]-120) ,Coordinates[i+1][2] ,120-(Coordinates[i+1][3]-120) , ST77XX_MAGENTA);

      //tft.drawLine(120+(120-Coordinates[i+1][2]), 120-(Coordinates[i+1][3]-120), 120+(120-Coordinates[i][2]), 120-(Coordinates[i][3]-120), ST77XX_BLUE);
      //tft.setCursor(120+(120-Coordinates[i+1][2]), (120-(Coordinates[i+1][3]-120)) - 10);
      //tft.println((i * 10)); 
    }
  }
